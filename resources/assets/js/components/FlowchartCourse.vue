<template>
    <div class="course list-group-item move" :class="{'custom-course': custom, 'complete-course': complete}" :key="course.id">
        <div class="pull-right">
            <div class="btn-group">
                <button type="button" class="prereqs btn btn-default btn-xs" aria-label="Prerequisites"
                    title="Show Prerequisites" v-if="course.course_id > 0" @!click="showPrereqs">
                    <i class="fa fa-arrows-alt" aria-hidden="true"></i>
                </button>
                <button type="button" class="edit-course btn btn-default btn-xs" aria-label="Edit"
                    title="Edit Course" @!click="editCourse">
        ...
        <div class="course-content pull-left">
            <template v-if="course.name !== null && course.name.length !== 0">
                <p>
                ...
                <p v-if="course.electivelist_name !== null && course.electivelist_name.length !== 0">
                    <i class="fa fa-code-fork text-primary" aria-hidden="true"></i> {{ course.electivelist_abbr }}
                </p>
            </template>
            <p v-else>
                <i class="fa fa-code-fork text-primary" aria-hidden="true"></i>
                <b>{{ course.electivelist_abbr }} ({{ course.credits }})</b>
            </p>
            <p v-if="course.completedcourse_name !== null && course.completedcourse_name.length !== 0">
                <i><i class="fa fa-check text-success" aria-hidden="true"></i> {{ course.completedcourse_name }}</i>
            </p>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            custom() {
                return this.course.degreerequirement_id === null;
            },
            complete() {
                return this.course.completedcourse_id !== null;
            },
        },
        methods: {
    ...
    function prereqClass() {
        ...
        let pre = false;
        let post = false;
        if (this.course.course_id !== null && this.selectedCousreId === this.course.course_id) {
            cur = true;
        }
