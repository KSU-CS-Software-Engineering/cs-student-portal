...
class ElectivelistsController extends Controller
{
    public function getElectivelists(Request $request, $id = -1)
    {
        if ($id < 0) {
            if ($request->has('deleted')) {
                ...
                return view('dashboard.electivelists')
                    ->with('electivelists', $electivelists)
                    ->with('page_title', "Deleted Elective Lists");
            } else {
                $electivelists = Electivelist::with('courses')->get();
                $deleted = Electivelist::onlyTrashed()->count();
                return view('dashboard.electivelists')
                    ->with('electivelists', $electivelists)
                    ->with('page_title', "Elective Lists")
                    ->with('deleted', $deleted);
            }
        } else {
            $electivelist = Electivelist::withTrashed()->findOrFail($id);
            return view('dashboard.electivelistedit')
                ->with('electivelist', $electivelist)
                ->with('page_title', "Edit Elective List");
        }
    }
    ...
    public function getElectivelistDetail(Request $request, $id = -1)
    {
        if ($id < 0) {
            ...
            $electivelist = Electivelist::withTrashed()->findOrFail($id);
            return view('dashboard.electivelistdetail')
                ->with('electivelist', $electivelist)
                ->with('page_title', "Elective List Details");
        }
    }
    ...
    public function getNewelectivelist()
    {
        ...
        return view('dashboard.electivelistedit')
            ->with('electivelist', $electivelist)
            ->with('page_title', "New Elective List");
    }
