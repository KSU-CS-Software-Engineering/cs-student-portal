...
class GroupsessionsController extends Controller
{
    public function getGroupsessions(Request $request, $id = -1)
    {
        if ($id < 0) {
            ...
            return view('dashboard.groupsessions')
                ->with('groupsessions', $groupsessions)
                ->with('page_title', "Groupsessions");
        } else {
            $groupsession = Groupsession::findOrFail($id);
            $groupsession->load('student', 'advisor');
            return view('dashboard.groupsessionedit')
                ->with('groupsession', $groupsession)
                ->with('page_title', "Edit Groupsession");
        }
    }
